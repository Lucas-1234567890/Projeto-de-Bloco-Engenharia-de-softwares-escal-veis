...
public class TaskService {
    ...
    public TaskService(TaskRepository repository, TaskHistoryRepository historyRepository) {
    ...
    public List<TaskHistory> historico(Long id) {
        // Não valida existência da task atual de propósito: uma task deletada
        // não existe mais em `tasks`, mas seu histórico continua consultável.
        List<TaskHistory> eventos = historyRepository.findByTaskIdOrderByChangedAtDesc(id);
        if (eventos.isEmpty()) {
    ...
    private void registrarHistorico(Task task, TaskAction action) {
        historyRepository.save(TaskHistory.of(task, action));
    }
}
